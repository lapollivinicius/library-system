<?php 
  ...
  class user {
    ...
    public function create(\entities\user $user) {
      ...
      $query = '
        INSERT INTO users (user_id, library_id, name, email, password, is_active)
        VALUES (:user_id, :library_id, :name, :email, :password, :is_active);
      ';
    ...
    public function read(string $email) {
      $query = '
        SELECT user_id, library_id, name, email, password, is_active
        FROM users
        WHERE email = :email
        LIMIT 1
      ';
